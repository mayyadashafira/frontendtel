...
import { MemoryStick, Plus, Eye, Pencil, Trash2, ChevronDown, ImagePlus } from "lucide-react";
...
function getStatusBadge(action) {
  ...
  if (normalized === "IN USE") return { className: "badge-good", label: normalized };
  if (normalized === "IN STORE") return { className: "badge-store", label: normalized };
  if (normalized === "BROKEN") return { className: "badge-broken", label: normalized };
        ...
        <>
          <div className="ram-topbar">
            ...
            <div className="ram-topbar-right">
              ...
              <CsvToolsBar
                isAdmin={isAdmin}
                exportFileName="ramAssets"
                data={assets}
                onImport={async (rows) => {
                  const created = await ramAssetService.bulkImport(rows);
                  setAssets((prev) => [...prev, ...created]);
                }}
              />
            </div>
          ...
          <section className="ram-section stagger-item">
                ...
                <tbody>
                  {filtered.map((a, i) => (
                    <tr key={a.id}>
                      ...
                      <td>
                        {(() => {
                          const status = getStatusBadge(a.action);
                          return (
