...
import { tabletAssetService, TABLET_STATUSES } from "../services/tabletAssetService";
...
export default function DeviceOfficeOutputTablet() {
  ...
  useEffect(() => {
    ...
    async function loadAssets() {
      setLoading(true);
      const data = await tabletAssetService.list();
      if (isMounted) {
        setAssets(data);
        setLoading(false);
      }
  ...
  function handleFormChange(field, value) {
    if (field === "action") {
      const statusInfo = TABLET_STATUSES.find((s) => s.value === value);
  ...
  async function handleSave() {
    ...
    try {
      if (isEdit) {
        const updated = await tabletAssetService.update(formData.id, formData);
        setAssets((prev) => prev.map((a) => (a.id === updated.id ? updated : a)));
      } else {
        const created = await tabletAssetService.create(formData);
  ...
  async function handleDelete() {
    ...
    setIsDeleting(true);
    try {
      await tabletAssetService.remove(deleteTarget.id);
      ...
      {loading ? (
        ...
        <>
          <div className="dashboard-top">
            <h1 className="tablet-title">Tablet</h1>
            ...
            <div className="tablet-header-actions">
              ...
              <CsvToolsBar
                isAdmin={isAdmin}
                exportFileName="tabletAssets"
                data={assets}
                onImport={async (rows) => {
                  const created = await tabletAssetService.bulkImport(rows);
                  setAssets((prev) => [...prev, ...created]);
                }}
              />
            </div>
                ...
                <tbody>
                  {filtered.map((a, i) => (
                    <tr key={a.id}>
                      ...
                      {isAdmin && (
                      <td>
                        <div className="tablet-action-group">
            ...
            <div className="dash-modal-body">
              ...
              <div className="dash-form-row">
                ...
                <div className="dash-form-group">
                  ...
                  <input
                    placeholder="..."
                    value={formData.charger_tab}
                    onChange={(e)=>
                      handleFormChange("charger_tab", e.target.value)
                    }
                  />
                </div>
              </div>
              ...
              <div className="dash-form-row">
                ...
                <div className="dash-form-group">
                  ...
                  <input
                    placeholder="..."
                    value={formData.casing_tab}
                    onChange={(e)=>
                      handleFormChange("casing_tab", e.target.value)
                    }
                  />
                </div>
              </div>
