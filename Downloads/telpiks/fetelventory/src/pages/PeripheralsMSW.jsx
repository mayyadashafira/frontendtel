...
import { mswAssetService, MSW_MANUFACTURERS, MSW_STATUSES } from "../services/mswAssetService";
...
export default function PeripheralsMSW() {
  ...
  function handleFormChange(field, value) {
    if (field === "action") {
      const statusInfo = MSW_STATUSES.find((s) => s.value === value);
  ...
  async function handleSave() {
    if (!formData.entity_id || !formData.serial_number || !formData.manufacture) {
      ...
      {loading ? (
        ...
        <>
          <div className="msw-topbar">
            <h1 className="msw-title">Mouse Wireless
            <br/>(MSW)</h1>
            ...
            <div className="msw-topbar-right">
              ...
              <CsvToolsBar
                isAdmin={isAdmin}
                exportFileName="mswAssets"
                data={assets}
                onImport={async (rows) => {
                  const created = await mswAssetService.bulkImport(rows);
                  setAssets((prev) => [...prev, ...created]);
                }}
              />
            </div>
